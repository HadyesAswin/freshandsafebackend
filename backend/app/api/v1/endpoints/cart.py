...
def get_cart(user_id: int, zipcode: Optional[str] = None, db: Session = Depends(get_db)):
    ...
    if zipcode and zipcode != "undefined" and zipcode.strip() != "":
        ...
        if u_lat and u_lng:
            ...
            if active_outlets:
                # ✅ THE FIX: Pool all nearby outlets instead of picking only index [0]
                outlet_ids = [o.id for o in active_outlets]
                
                # Fetch the MAXIMUM stock available for each product across all nearby outlets
                shop_prods = db.query(
                    ShopProduct.product_id, 
                    func.max(ShopProduct.stock)
                ).filter(
                    ShopProduct.outlet_id.in_(outlet_ids),
                    ShopProduct.is_available == True,
                    ShopProduct.stock > 0 
                ).group_by(ShopProduct.product_id).all()
...
def validate_guest_cart(data: GuestCartRequest, db: Session = Depends(get_db)):
    ...
    if data.zipcode and data.zipcode != "undefined" and data.zipcode.strip() != "":
        ...
        if u_lat and u_lng:
            ...
            if active_outlets:
                # ✅ THE FIX: Pool all nearby outlets for Guest users too!
                outlet_ids = [o.id for o in active_outlets]
