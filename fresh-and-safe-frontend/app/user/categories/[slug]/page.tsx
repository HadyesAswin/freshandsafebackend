...
export default function CategoryProductPage() {
  const params = useParams(); // Captures the slug (e.g., "vegetables")
  ...
  useEffect(() => {
    // 1. Get Zipcode (Strict Logic: Must have zipcode to see products)
    ...
    // 2. Fetch Products for this Category + Zipcode
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        {!loading && products.length === 0 && (
            <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
                ...
                <Link href="/" className="inline-block mt-6 px-6 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700">
                    Browse other items
                </Link>
            </div>
        )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {products.map((product) => (
            <div key={product.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                ...
                <div className="p-5">
                    ...
                    <button className="w-full py-2 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 active:scale-95 transition-all shadow-md text-sm">
                        Add to Cart
                    </button>
                </div>
            </div>
