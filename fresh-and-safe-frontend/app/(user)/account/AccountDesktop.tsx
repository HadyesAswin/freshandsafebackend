          ...
          if (data && data.address && data.address.postcode) {
              ...
              if (mappedZip !== sessionZip) {
                  if (mappedZip.substring(0, 4) === sessionZip.substring(0, 4)) {
                      ...
                      setLocationError(`Mismatched Location! The map pin (${mappedZip}) is too far from your selected store area (${sessionZip}).`);
                      return; 
                  }
              } else {
                  setPinVerified(true);
            ...
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4 border-b pb-4">
                ...
                {!showAddForm && !editingAddress && (
                  <button onClick={() => {setShowAddForm(true); setPinVerified(null)}} className="bg-[#00b8d9] hover:bg-[#00a0bd] text-white font-bold py-3 px-6 rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-cyan-100">
              ...
              {showAddForm && (
                <div className="bg-white p-8 rounded-2xl border border-slate-200 mb-10 shadow-sm animate-in slide-in-from-top duration-300">
                  ...
                  <div className="mb-6 p-5 bg-slate-50 border border-slate-200 rounded-xl">
                      <p className="text-xs font-bold text-slate-500 mb-4 uppercase tracking-widest">Step 1: Set Exact GPS Location</p>
                      <button type="button" onClick={() => handleGetExactLocation(false)} disabled={isLocating} className="w-full mb-4 py-3.5 border-2 border-dashed rounded-xl flex items-center justify-center gap-2 font-bold transition-all bg-white text-[#00b8d9] border-[#00b8d9]/30 hover:bg-cyan-50">
              ...
              {editingAddress && (
                <div className="bg-white p-8 rounded-2xl border border-slate-200 mb-10 shadow-sm animate-in slide-in-from-top duration-300">
                  ...
                  <div className="mb-6 p-5 bg-slate-50 border border-slate-200 rounded-xl">
                      ...
                      <button type="button" onClick={() => handleGetExactLocation(true)} disabled={isLocating} className="w-full mb-4 py-3.5 border-2 border-dashed rounded-xl flex items-center justify-center gap-2 font-bold transition-all bg-white text-[#00b8d9] border-[#00b8d9]/30 hover:bg-cyan-50">
                  ...
                  {loadingData ? <p className="text-slate-500 col-span-2">Loading addresses...</p> : addresses.length === 0 ? (
                    ...
                    <div key={addr.id} className="bg-white rounded-2xl p-6 border border-slate-200 relative group hover:border-[#00b8d9] transition-colors shadow-sm">
                      <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => {setEditingAddress(addr); setPinVerified(true);}} className="text-[10px] font-black uppercase text-[#00b8d9] bg-cyan-50 px-3 py-1 rounded-md hover:bg-cyan-100 transition-colors">Edit</button>
