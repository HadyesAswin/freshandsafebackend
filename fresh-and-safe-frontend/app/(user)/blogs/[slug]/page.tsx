...
export default function BlogDetailPage() {
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/v1/blogs/${slug}`);
  ...
  if (!blog) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
      ...
      <Link href="/blogs" className="text-green-600 font-bold hover:underline">Return to Blogs</Link>
    </div>
  );
    ...
    <main className="min-h-screen bg-white pb-20 font-sans">
      <header className="bg-white/80 backdrop-blur-md border-b sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <Link href="/blogs" className="flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-green-600 transition-colors">
